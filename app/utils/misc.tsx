...
function sanitizeRUT(rut: string): string {
	// Remove all non-numeric characters except 'K' (in case the RUT already contains a DV)
	return rut.replace(/[^0-9K]+/gi, '')
}

export function calculateDV(rut: string): string {
	const cleanRut: string = sanitizeRUT(rut) // Ensure the RUT is clean before processing
	const reversedNumbers: number[] = cleanRut.split('').reverse().map(Number)
	const sequence: number[] = [2, 3, 4, 5, 6, 7]
	let total: number = 0

	reversedNumbers.forEach((number, index) => {
		total += number * sequence[index % sequence.length]
	})

	const remainder: number = total % 11
	const result: number = 11 - remainder
