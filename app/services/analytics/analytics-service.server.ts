...
export async function getDailyTransactionsForWeek({
	businessId,
	...
	const dailyTransactions = await Promise.all(
		daysRange.map(async (day) => {
			...
			const sales = orders
				.flatMap((order) => order.productOrders)
				.filter((productOrder) => productOrder.type !== ProductOrderType.RETURN)
				.map((productOrder) => productOrder.quantity)
	...
	const returns = dailyTransactions.reduce((acc, transaction) => acc + transaction.returns, 0)
	const sales = dailyTransactions.reduce((acc, transaction) => acc + transaction.sales, 0)

	const returnPercentage = (returns / (sales + returns)) * 100

	return { dailyTransactions, weekStartDate: start, weekEndDate: end, returnPercentage }
}
